...
class ANTA:
    def __init__(self, n: int, colony_size=15, alpha=1.0, beta=3.0, rho=0.1, initial_pheromone=1.0, steps=150) -> None:
    ...
    def ant_find_tour(self):
        ...
        while len(tour) < self.num_nodes:
            ...
            if len(available_neighbors) == 0: return [], float('inf')
    ...
    def update_pheromones(self, tours, best_cost):
        for tour in tours:
            if tour is None: continue
            for i in range(len(tour) - 1):
                edge = (tour[i], tour[i + 1])
                delta_tau = self.initial_pheremone / best_cost # using best cost for more informed updates
                self.pheromones[edge] = (1 - self.rho) * self.pheromones[edge] + delta_tau
